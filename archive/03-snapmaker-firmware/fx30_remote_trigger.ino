...
// Ignore a second trigger within this window. The print head can bounce the
// lever, brush it twice on the way past, or wobble the rig enough to close it
// again. Debounce alone does not cover that: those are real, separate presses
// milliseconds apart. Raise it if you still get doubles, but keep it below
// your fastest layer time.
#define OC_RETRIGGER_LOCKOUT_MS 1500

// Wait this long after the switch closes before firing the shutter.
//
// In a print timelapse the head PRESSES the switch and then MOVES AWAY, so
// that it is out of the camera's frame when the picture is taken. Firing the
// shutter the instant the contact closes would photograph the toolhead sitting
// right over the print, on every single frame.
//
// Set it to roughly (press dwell + travel to the rest position + a little
// settling). For the Klipper macro's 100 ms press and short back-off move,
// 600 ms lands the shot with the head parked and the machine still.
// Set to 0 for handheld use, where you want the shutter to be instant.
...
static void serviceButtons() {
  ...
  if (btnService(gPhotoBtn, 0) == BTN_TAP) {                 // limit switch
    ...
    if (now - gLastTrigger < OC_RETRIGGER_LOCKOUT_MS && gLastTrigger != 0) {
      Serial.printf("[%lu] trigger ignored, only %lu ms since the last one\n",
                    (unsigned long)now, (unsigned long)(now - gLastTrigger));
    } else if (gState != ST_READY) {
      setStatus("no camera");
      Serial.println("!! trigger MISSED - camera not connected");
    } else {
      gLastTrigger = now;
      gFrames++;
#if OC_TRIGGER_DELAY_MS > 0
      // Let the head retreat out of frame and the machine stop moving before
      // the shutter opens. pumpEvents keeps the PTP session healthy meanwhile.
      Serial.printf("[%lu] triggered, waiting %d ms for the head to clear\n",
                    (unsigned long)now, OC_TRIGGER_DELAY_MS);
      setStatus("waiting...", OC_TRIGGER_DELAY_MS + 200);
      draw();
      for (int i = 0; i < OC_TRIGGER_DELAY_MS / 20; i++) { pumpEvents(); delay(20); }
#endif
      doPhoto();
      Serial.printf("[%lu] frame %lu\n", (unsigned long)millis(), (unsigned long)gFrames);
    }
  }
}
...
void loop() {
  ...
  if (millis() - lastBeat > 5000) {
    lastBeat = millis();
    Serial.printf("[%lu] state=%d wifi=%d cmd=%d rec=%d frames=%lu rssi=%d\n",
                  (unsigned long)millis(), gState, WiFi.status()==WL_CONNECTED,
                  cmdSock.connected(), gRecording, (unsigned long)gFrames,
