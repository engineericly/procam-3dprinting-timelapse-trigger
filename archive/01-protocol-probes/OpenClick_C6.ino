/*
 * OpenClick C6 - Sony FX30 remote over PTP/IP
 *
 * Board  : Seeed Studio XIAO ESP32-C6
 * Button : external 3-pin limit switch (microswitch) on D2 / GPIO2
 * Camera : ILME-FX30 via Wi-Fi Direct, PC Remote / PTP-IP on port 15740
 *
 * WIRING - 3-pin limit switch
 *   A microswitch has three tags: C (common), NO (normally open), NC
 *   (normally closed). Use C and NO only:
 *
 *      switch C  ------> GND
 *      switch NO ------> D2 (GPIO2)
 *      switch NC ------> leave unconnected
 *
 *   The pin uses the internal pull-up, so it idles HIGH and reads LOW when
 *   the lever is pressed. No external resistor. If you wire NC instead of NO
 *   the logic inverts and it will fire once at boot and never again - set
 *   OC_BUTTON_ACTIVE_LOW to 0 if you would rather keep that wiring.
 *
 * ANTENNA - this matters on the XIAO C6
 *   The C6 has a ceramic antenna and a u.FL connector behind an RF switch.
 *   GPIO3 (WIFI_ENABLE) must be driven LOW to power that switch, and GPIO14
 *   (WIFI_ANT_CONFIG) picks which antenna. Leaving them floating gives weak
 *   or absent Wi-Fi, which looks like a camera problem but is not.
 *   Handled in setup(); set OC_USE_EXTERNAL_ANTENNA if you fit a u.FL one.
 *
 ...
 *
 * Libraries: U8g2 by oliver - only if OC_USE_OLED is 1. The XIAO C6 has no
 * screen, so it defaults to 0 and status goes to the on-board LED instead.
 *
 * Board package: esp32 by Espressif 3.x, board "XIAO_ESP32C6",
...
// --- XIAO ESP32-C6 pin map (from the board's own pins_arduino.h) ----------
//   D0=GPIO0  D1=GPIO1  D2=GPIO2  D3=GPIO21 D4=GPIO22(SDA) D5=GPIO23(SCL)
//   D6=GPIO16 D7=GPIO17 D8=GPIO19 D9=GPIO20 D10=GPIO18
//   LED_BUILTIN=GPIO15, WIFI_ENABLE=GPIO3, WIFI_ANT_CONFIG=GPIO14
// GPIO4/5/8/9/15 are strapping pins on the C6 - do not hang a switch on them.
#define OC_BUTTON_PIN        D2      // GPIO2, free and not a strapping pin
#define OC_BUTTON_ACTIVE_LOW 1       // switch C to GND, NO to the pin

// The XIAO has no display. Set to 1 and wire an SSD1306 to D4/D5 if you add one.
#define OC_USE_OLED   0
#define OC_OLED_SDA   D4     // GPIO22
#define OC_OLED_SCL   D5     // GPIO23

// Status LED, used whenever there is no OLED. On XIAO boards this LED is
// wired active LOW: writing LOW turns it ON.
#define OC_USE_LED     1
#define OC_LED_PIN     LED_BUILTIN
#define OC_LED_ACTIVE_LOW 1

// 0 = on-board ceramic antenna, 1 = external u.FL antenna.
#define OC_USE_EXTERNAL_ANTENNA 0

#if OC_USE_OLED
  #include <U8g2lib.h>      // must come after OC_USE_OLED is defined
#endif
...
// Status on the on-board LED when there is no screen:
...
static void ledStatus() {
  ...
  bool on = false;
  uint32_t t = millis();
  if (gState != ST_READY)      on = (t % 300) < 150;            // fast blink
  else if (gRecording)         on = (t % 1000) < 120 || ((t % 1000) > 200 && (t % 1000) < 320);
  else                         on = true;                        // solid
...
#else
  bool raw = (digitalRead(OC_BUTTON_PIN) == HIGH);
#endif
  uint32_t now = millis();
...
#endif
  // The XIAO C6 routes its radio through an RF switch that is OFF until GPIO3
  // is driven LOW. Skip this and Wi-Fi is weak or dead - and it looks exactly
  // like a camera or password fault, which is a miserable thing to debug.
  pinMode(WIFI_ENABLE, OUTPUT);
  digitalWrite(WIFI_ENABLE, LOW);              // enable the RF switch
  delay(100);
  pinMode(WIFI_ANT_CONFIG, OUTPUT);
  digitalWrite(WIFI_ANT_CONFIG, OC_USE_EXTERNAL_ANTENNA ? HIGH : LOW);
  delay(100);

#if OC_BUTTON_ACTIVE_LOW
  pinMode(OC_BUTTON_PIN, INPUT_PULLUP);        // switch shorts the pin to GND
#else
  pinMode(OC_BUTTON_PIN, INPUT_PULLDOWN);
#endif
...
void loop() {
  ...
  if (millis() - lastBeat > 5000) {
    lastBeat = millis();
    Serial.printf("[%lu] state=%d wifi=%d cmd=%d mode=%s rec=%d sw=%d\n",
