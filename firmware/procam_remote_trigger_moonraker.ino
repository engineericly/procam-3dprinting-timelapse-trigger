/*
 * OpenClick C3 WiFi - PROCAM remote over Sony PTP/IP, Moonraker-triggered
 *
 ...
 * Camera : PROCAM (currently a Sony ILME-FX30) via a SHARED WI-FI ROUTER,
 *            PC Remote / PTP-IP on port
...
// Point PROCAM's own Wi-Fi settings at this same router too (camera menu:
// Network > Wi-Fi Settings > join the router, not "PC Remote"/Wi-Fi Direct).
// Wi-Fi and host config lives in secrets.h, which is gitignored. Copy
// secrets.example.h to secrets.h and put your real values there. The
// placeholders below keep the sketch compiling if you have not yet.
...
// Strongly recommended: give PROCAM a DHCP reservation on your router and
...
#  define MOONRAKER_HOST "192.168.1.50"   // <-- your printer's IP, or set it in secrets.h
#endif
...
static void draw() {
  ...
  switch (gState) {
    ...
    case ST_READY:     top = gMoonrakerConnected ? "PROCAM+MR" : "PROCAM";  break;
...
static void sendMoonrakerIdentify() {
  ...
  snprintf(buf, sizeof(buf),
    ...
    "\"params\":{\"client_name\":\"procam-esp32\",\"version\":\"1.0.0\","
    "\"type\":\"agent\",\"url\":\"https://github.com/local/procam-esp32\"},"
    "\"id\":1}");
  wsClient.sendTXT(buf);
}

// Subscribes to display_status.message - the field M117 writes to. Used
// instead of RESPOND/notify_gcode_response because RESPOND needs a
// [respond] section in printer.cfg that isn't present on every Klipper
// install (confirmed missing on a Creality K2 Pro's stock config - it
// answered "Unknown command:RESPOND"). M117 comes from a different, more
// commonly-enabled module ([display_status], which Fluidd/Mainsail's own
// status bar already depends on) and writes into this subscribed object
// instead of echoing through the gcode response channel, so it needs its
// own subscription rather than the notify_gcode_response scan below.
static void sendMoonrakerSubscribe() {
  static char buf[160];
  snprintf(buf, sizeof(buf),
    "{\"jsonrpc\":\"2.0\",\"method\":\"printer.objects.subscribe\","
    "\"params\":{\"objects\":{\"display_status\":[\"message\"]}},"
    "\"id\":2}");
  wsClient.sendTXT(buf);
}
...
static void webSocketEvent(WStype_t type, uint8_t* payload, size_t length) {
  switch (type) {
    ...
    case WStype_TEXT: {
      ...
      // can see exactly what Moonraker is sending, instead of only ever
      // logging on a match. Remove once triggering is confirmed working -
      // this fires on every keepalive-adjacent status push too, not just
      // ones we care about, so it's noisy by design.
      Serial.printf("   WS TEXT (%u bytes): %.220s\n", (unsigned)length, (const char*)payload);
      ...
      // library dependency.
      //
      // Watches for the PUSH notification Moonraker sends when a
      // subscribed object changes ("notify_status_update"), not the direct
      // reply to our own subscribe request above (that reply carries
      // "result"/matching "id", never "notify_status_update", so it can't
      // false-trigger on whatever display_status.message happened to be
      // set to at subscribe time).
      if (strstr((const char*)payload, "notify_status_update") &&
          strstr((const char*)payload, "display_status") &&
          strstr((const char*)payload, TRIGGER_TOKEN)) {
        Serial.println("   trigger seen in Moonraker display_status message");
...
void loop() {
  ...
  // PROCAM closes an idle PTP/IP session after about 50 seconds. A cheap
